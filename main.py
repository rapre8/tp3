'''Permet de jouer au jeu au mode spécifié par l'utilisateur'''
import argparse
...
def mode_manuel():
    '''Permet de jouer manuellement contre le serveur de jeu'''
...
def décortiquer_coup_auto(pré_coup, post_coup):
    '''Permet d'analyser le coup choisi par l'algorithme du mode automatique'''
...
def mode_automatique():
    '''Permet de jouer automatiquement contre le serveur de jeu'''
    tuple_id_état = débuter_partie(analyser_commande.idul)
...
def afficher_damier_ascii(gamestate):
    '''Permet d'afficher le damier pour le mode manuel'''
